       ...
       run through 180 deg and flipped, and every car has it in one of two layouts. */
{
  ...
  check(side + " with wing pods, " + centre + " with an interior mirror, " + frames.size + " housing styles",
    bad.length === 0 && side === 13 && centre === 37 && frames.size >= 6, bad.slice(0, 4).join(" | "));
}
   ...
   marque on the badge. */
{
  ...
  const MARQUE = {
    ...
    "Cadillac": "Cadillac", "Red": "Red Bull", "Alfa": "Alfa Romeo", "SSC": "SSC", "Gordon": "Gordon Murray",
  };
  const bad = [];
  let checked = 0;
  for (const f of readdirSync(ROOT).filter((x) => /simulator\.html$/i.test(x))) {
...
console.log("▶ garage");
check("fifty car cards render", await page.locator(".car-card").count() === 50);
...
check(`every card key discovered from the page (${CAR_KEYS.length})`, CAR_KEYS.length === 50);
