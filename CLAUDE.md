...
Rolls-Royce Spectre Black Badge simulator.html unchanged: Phantom VIII (6.75 TT V12, 2,560 kg, 250 governed)
                                               ...
                                               rear compartment on keys 1-0, and Chauffeur 101 in place of
                                               Race Car 101. The Phantom is NOT on the template at all — it is
                                               the owner's own HTML — and choosing London puts you on the real
                                               ROAD NETWORK, 20 km round Charing Cross, junction by junction.
Ferrari F12tdf simulator.html               ← a 2014 F12berlinetta rebuilt at Maranello as an F12tdf DEVELOPMENT
                                               PROTOTYPE: 6.3 F140 FC V12, 780 CV @ 8,500, 8,900 rpm, and no
                                               homologation — so it moves on a development ECU pegged at
...
their request, with a certification adapter so `perf-test` still certifies it (5.4000 s / 250
km/h / 38.00 m). `tests/browser-test.mjs` carries `OWNERS_OWN` and answers seven template
guards on it explicitly. It is generated by five scripts run in order from the repo root —
`build_phantom.py` → `build_world.py` → `build_wire.py` → `build_guards.py` →
`build_london.py` — and **a great deal has since been applied inline on top of them**, so
re-running the earlier stages destroys work.

Choosing **London** in Navigation does not set a route: it puts you on a real street at a real
junction on a 118-node / 174-road hand-drawn graph reaching 20 km out from Charing Cross, and
then leaves you alone. There is no line to be pulled back onto because there is no line. Five
things that were wrong about it, and are the kind of thing to check on any street car:

- **`driveSide()` read only the LANE PLANS**, so it was right about the eight fixed journeys
  and silently returned +1 — the right-hand side — for London, which has no lane plan because
  every road on the network carries its own. That one number is upstream of a great deal:
  `homeLaneOffAt` put you in the right-hand lane of a British street, the ambient traffic drove
  on the right, and lane centring dragged you across the road toward all of it. It now asks the
  destination, which declares its own `drive`.
- **Lane centring worked in multiples of three metres** clamped to ±1 — not a lane on any road
  in the plan. On a 16 m street with two lanes each way the nearside centre is about −6.2 m, so
  "hold the lane you are in" rounded that to −3 and steered three metres toward the oncoming
  traffic and held it there. That is *the car driving itself sideways with the chauffeur off*,
...
- **A junction was real but invisible.** The turn always worked — turn the wheel and you came
  ...
  cluster the whole junction; `drawJunctionGround` opens a real mouth of tarmac per road at that
  road's **real bearing and real width**, and `drawJunctionPost` stands a fingerpost on the
  country's kerb with a blade per road, the one your wheel is asking for picked out in gold.
  A live strip in the binnacle lists ← ↑ → with the road names, and it changes under the wheel
  while you approach, so you may change your mind up to the line.
- **A network road built no frontage at all.** The eight city kits pin their blocks to absolute
  distances ("citylondon, 340 m to 1400 m") and a network road is typically two or three hundred
  metres, so nearly every one of them came out bare. `londonFrontage()` builds a road's own
  frontage from what that road really is — the district at each end (`LDN_LOOK`), the open ground
  beside it (`LDN_OPEN`: Hyde Park down Park Lane, the Thames off the Embankment), a gap at both
  ends because the ends are junction boxes, and a stretch carried **past** the junction so the
  view across a crossroads is a street rather than a hole. `LDN_MARKS` stands the landmarks at
  the junctions they really stand at, so you find Nelson, the Palace and Big Ben by driving at them.
- **`drawRoad` filled the whole ground below the horizon with `pal.grass`** — after the "back"
  pass had drawn the river. So the Thames was painted over every frame, which is why Tower Bridge
  had grey ground on both sides and appeared to vanish as you reached it. On a street the caller
  lays the ground itself and passes `skipGround`. (Same fix in the Spectre.)

**Tower Bridge** was also drawn as a *landmark*, and `drawLandmark` refuses anything with a
projected scale over 12 — right for a building beside the road, exactly wrong for a structure you
drive through, so the towers disappeared at about twenty metres. It is now part of the road: a
real edge of the network, 8.2 m between the towers with one lane each way, the Thames drawn on
both sides and **nine metres below the deck** (`drawBridgeWater`, under the deck you look at it
from), the two Gothic towers with their high-level walkways, the suspension chains, and bascules
that lift for a vessel on their own clock. While they are up there is a **gap**, and what happens
at it is arithmetic: `bridgeJump(v, open)` is projectile motion off a 1-in-11 ramp raised 43°, so
your reach at the lip is compared with the gap and you clear it, land in the river, or land on a
vessel — and whether the vessel is survivable depends on how hard you hit it. The number is on
